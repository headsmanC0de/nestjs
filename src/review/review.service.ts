...
import type { ReviewsModel, ReviewsDocument } from './review.model';
...
export class ReviewService {
    ...
    async create(dto: CreateReviewDto): Promise<ReviewsDocument> {
        const reviewModel = new this.reviewsModel(dto);
    ...
    async findByProductId(productId: string): Promise<ReviewsDocument[]> {
        return this.reviewsModel.find({ productId: new Types.ObjectId(productId) }).exec();
    }
